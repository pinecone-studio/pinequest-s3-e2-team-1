...
export default function StudentApp() {
  ...
  const [progress, setProgress] = useState<GetProgressResponse | SubmitAnswersResponse | null>(null);
  ...
  const fetchStudents = async () => {
    try {
      const { students } = await gqlRequest(`query { students { id name className } }`);
  ...
  const filteredTests = Array.from(
    tests
      .filter((t) => {
        ...
        const topicMatch = t.criteria.topic.toLowerCase().includes(searchQuery.topic.toLowerCase());
        if (!topicMatch) return false;

        // 3. Filter out if already submitted by this student
        const alreadySubmitted = allAttempts.some(
          (a) => a.studentId === selectedStudent.id && a.testId === t.id && (a.status === "submitted" || a.status === "approved")
        );
      ...
      .reduce((map, t) => {
        const key = `${t.criteria.subject}-${t.criteria.topic}-${t.title}`.toLowerCase();
  ...
  const handleSubmit = (finalize: boolean) => {
    ...
    startTransition(async () => {
      try {
        const inputAnswers = Object.entries(answers).map(([questionId, selectedOptionId]) => ({
          questionId,
          selectedOptionId,
        }));
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-slate-950 text-white">
        <div className="rounded-xl border border-rose-500/20 bg-rose-500/10 p-8 text-center">
          ...
          <button onClick={() => window.location.reload()} className="mt-4 rounded-lg bg-rose-500 px-6 py-2">
            Дахин ачаалах
          </button>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-slate-950 px-6 py-12 text-slate-100 font-sans">
      <main className="mx-auto max-w-4xl space-y-8">
        <header className="space-y-4">
          <p className="text-xs uppercase tracking-[0.4em] text-cyan-400">Шалгалт Өгөх Портал (Порт 3002)</p>
          <h1 className="text-4xl font-bold tracking-tight sm:text-5xl">Оюутны Шалгалтын Танхим</h1>
          <p className="text-slate-400">Энд зөвхөн оюутнууд тестээ бөглөж, дүнгээ харах боломжтой.</p>
        </header>
          ...
          <section className="space-y-6">
            <div className="grid gap-4 rounded-2xl border border-white/10 bg-white/5 p-6 md:grid-cols-2">
              <div className="space-y-2">
                <label className="text-xs uppercase tracking-widest text-slate-500">Оюутан Сонгох (ID-гаар)</label>
                <select
              ...
              <div className="space-y-2">
                <label className="text-xs uppercase tracking-widest text-slate-500">Сэдэвээр хайх</label>
                <input
                  className="w-full rounded-xl border border-white/10 bg-black/20 px-4 py-3 outline-none focus:ring-1 focus:ring-cyan-500"
                  placeholder="Бүгд"
                  value={searchQuery.topic}
                  onChange={(e) => setSearchQuery((s) => ({ ...s, topic: e.target.value }))}
                />
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                {filteredTests.length === 0 ? (
                  <p className="text-slate-500 italic">Таны оруулсан бүлэг, сэдэвт тохирох тест олдсонгүй.</p>
                ) : (
                  filteredTests.map((t) => (
                    <div key={t.id} className="rounded-2xl border border-white/10 bg-white/5 p-6 hover:bg-white/8 transition duration-300">
            ...
            {progress?.status === "submitted" && (
              <div className="rounded-3xl border border-emerald-500/20 bg-emerald-500/10 p-12 text-center space-y-4 animate-in zoom-in duration-500">
                ...
                <h2 className="text-3xl font-bold text-emerald-400">Шалгалт амжилттай дууслаа!</h2>
                <p className="text-slate-400 text-lg">Таны хариултуудыг хүлээн авлаа. Багш таны дүнг баталсны дараа та үр дүнгээ харах боломжтой болно.</p>
                <button
            ...
            {allAttempts.some(a => a.studentId === selectedStudent?.id && a.status === "approved") && (
              <div className="space-y-4 pt-8">
                <h2 className="text-xl font-semibold text-emerald-400">Миний батлагдсан дүнгүүд</h2>
                <div className="grid gap-4">
                  {allAttempts.filter(a => a.studentId === selectedStudent?.id && a.status === "approved").map(a => (
                    <div key={a.attemptId} className="rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-6 animate-in fade-in duration-300">
                      <div className="flex justify-between items-center">
                        <div>
                          <h3 className="text-lg font-bold">{a.title}</h3>
                          <p className="text-xs text-slate-500">{new Date(a.submittedAt || "").toLocaleDateString()}</p>
                        </div>
                        <div className="text-right">
                          <p className="text-2xl font-black text-emerald-400">{a.score}/{a.maxScore}</p>
                          <p className="text-[10px] uppercase font-bold text-slate-500">{a.percentage}% АМЖИЛТ</p>
                        </div>
                      </div>
                      <div className="mt-4 space-y-2 max-h-[200px] overflow-y-auto pr-2 custom-scrollbar">
                        {a.result?.questionResults.map((r, i) => (
                          <div key={r.questionId} className={`flex items-center gap-3 p-2 rounded-lg text-xs ${r.isCorrect ? "bg-emerald-500/10 text-emerald-400" : "bg-rose-500/10 text-rose-400"}`}>
                            <span className="font-bold">{i + 1}.</span>
                            <span>{r.isCorrect ? "Зөв хариулсан" : "Алдсан"}</span>
                            {r.explanation && <span className="opacity-50 italic">- {r.explanation}</span>}
                          </div>
                        ))}
                      </div>
                    </div>
          ...
          <section className="space-y-8">
            <div className="flex items-center justify-between rounded-2xl border border-white/5 bg-white/5 p-6 backdrop-blur-lg">
              ...
              <div className="hidden sm:block text-right">
                <p className="text-xs uppercase text-slate-500 mb-1">Оролдлогын Seed ID</p>
                <code className="rounded bg-black/40 px-3 py-1 text-xs font-mono text-cyan-500 select-all">{attempt.attemptId}</code>
              </div>
            </div>

            <div className="space-y-6">
              {attempt.exam.questions.map((q, idx) => (
                <div key={q.questionId} className="rounded-3xl border border-white/10 bg-black/40 p-8 space-y-6 shadow-xl">
                  <div className="flex justify-between items-start">
                    <h3 className="text-xl font-medium leading-relaxed max-w-[85%]">
                      <span className="mr-4 text-cyan-400 font-bold">{idx + 1}.</span> {q.prompt}
                    </h3>
                    <span className="text-xs bg-cyan-900/50 text-cyan-200 px-3 py-1 rounded-full">{q.points} оноо</span>
                  </div>
                  <div className="grid gap-3">
                    {q.options.map((opt) => (
                      ...
                      >
                        ...
                        />
                        <div className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-full border-2 transition-colors ${answers[q.questionId] === opt.id ? "border-cyan-400 bg-cyan-400" : "border-slate-600"
                          }`}>
                          {answers[q.questionId] === opt.id && <div className="h-2 w-2 rounded-full bg-slate-950" />}
                        </div>
